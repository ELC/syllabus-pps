...
function locateForkRegionInExpanded(
  ...
  let afterIdx = afterSwapsWithLane
    ? expanded.indexOf(swap.swapWith)
    : expanded.indexOf(fork.after);
...
function separateRangeOnSpineOrder(
  ...
  const forkAfter =
    soonerIdx === 0 ? spine[0]! : resolveNewForkAfterAnchor(curation, spine, soonerIdx);
...
export function shiftConceptInOrder(
  ...
  if (
    ...
    return shiftCompressedTrunkSpinePair(curation, title, swapWith, direction);
  }
